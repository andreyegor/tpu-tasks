...
SELECT format('%s %s.', first_name, substr(last_name, 1, 1)) as shorten_full_name
FROM hr.employees
WHERE salary * 6 * 1.1 > 30000;
...
SELECT first_name || ' ' || substr(last_name, 1, 3) as some_sorta_name,
    CASE
...
FROM hr.employees
WHERE LOG(10, salary) = FLOOR(LOG(10, salary));
SELECT first_name,
    random() as rnd
FROM hr.employees
ORDER BY rnd;