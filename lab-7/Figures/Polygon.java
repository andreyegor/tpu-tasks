...
public class Polygon implements ClosedFigure {
    ...
    public Polygon(Vector point1, Vector point2, Vector... points) {
        this(Stream.concat(Stream.of(point1, point2), Stream.of(points))
                .map(v -> v.sub(point1)).toArray(Vector[]::new));
    }

    private Polygon(Vector[] points) {
        this(Stream.of(points).reduce(Vector::add).get().mul(1.0 / points.length), points);
    }

    private Polygon(Vector center, Vector[] points) {
        this(center,
                sort(Stream.of(points).map(v -> v.sub(center))).toArray(Vector[]::new),
                perimeter(points), area(points));
    }

    private Polygon(Vector Oxy, Vector[] points, double perimeter, double area) {
        this.Oxy = Oxy;
        this.points = points;
    ...
    public Polygon move(Vector to) {
        return new Polygon(Oxy.add(to), points, perimeter, area);
    }

    public Polygon scale(double k) {
        var scaledPoints = Stream.of(points).map(v -> v.mul(k)).toArray(Vector[]::new);
        return new Polygon(Oxy, scaledPoints, perimeter * k, area * k * k);
    }
